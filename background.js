...
adblock_is_paused = function (newValue)
{
	...
	if (SAFARI &&
		newValue === true && !get_settings().safari_content_blocking) {
		safari.extension.removeContentScript(safari.extension.baseURI + "adblock_safari_beforeload.js");
	} else if (SAFARI &&
		newValue === false && !get_settings().safari_content_blocking) {
	...
	updateButtonUIAndContextMenus = function ()
	{
		...
		function setBrowserButton(info)
		{
			...
			if (adblock_is_paused()) {
				...
				setBrowserActions(browsersBadgeOptions);
			} else if (info.disabled_site && !/^chrome-extension:.*pages\/install\//.test(info.tab.unicodeUrl)) {
				...
				browsersBadgeOptions.iconPaths = {
					'19': "img/icon19-whitelisted.png",
					'38': "img/icon38-whitelisted.png"
				};
				setBrowserActions(browsersBadgeOptions);
			} else {
...
get_content_script_data = function (options, sender)
{
	...
	if (hiding &&
		_myfilters &&
		_myfilters.hiding &&
		settings && !settings.safari_content_blocking) {
...
if ((localStorage['ga_installTime'] === undefined) || (localStorage['ga_UUID'] === undefined))
{
    setTimeout( function() { _myfilters.init(); }, 5000);
}else
{
    _myfilters.init();
}
	...
	if (!SAFARI &&
		...
		chrome.tabs.query({active: true, currentWindow: true}, function (tabs)
		{
			...
			var notificationOptions = {
				...
				buttons: [{
					title: translate('malwarenotificationlearnmore'),
			...
			if (OPERA) {
				...
				{
					if (buttonIndex === 0) {
						openTab("https://chrome.google.com/webstore/");
					}
					if (buttonIndex === 1) {
...
if (!SAFARI) {
	var runChannelWhitelist = function (tabUrl, tabId)
	{
		if (parseUri(tabUrl).hostname === "www.youtube.com" &&
			get_settings().youtube_channel_whitelist && !parseUri.parseSearch(tabUrl).ab_channel) {
